...
客户端可以设置 Read Preference从而可以从备份节点读取数据
...
仲裁者:不备份数据, 只适用于投票
一般的配置是 一个主节点 两个备份节点
v3.0.0之后 一个副本集可以有50个成员, 但最多只有7个成员具有投票权(否则可能会很难决策出谁能成为主节点)
## 其他属性 ##
优先级 隐藏 延迟
...
指定w=2或w=majority或其他自定义的值

# Replica Set Read and Write Semantics #
## Write Concern for Replica Sets ##
如果w是一个int, 那么会保证至少写入w台服务器(包括主节点在内)
如果w是一个string
	如果=majority, 那么会写入大部分的服务器才算成功
...
2. 随便连接到其中一个节点, 然后执行rs.initiate()
3. 用 rs.conf() 检查一下初始的配置
4. 通过rs.add('ip:port')的方式将其他的节点加入进来, 加的过程中可能会引发选举, 从而导致你连接的不再是主节点, 这要注意
...
2. mongod --port 27017 --dbpath /srv/mongodb/rs0-0 --replSet rs0 --smallfiles --oplogSize 128
	...
	3. --smallfiles额--oplogSize可以减少磁盘占用
3. 随便连接到一个mongoed
...
3. rs.initiate() rs.conf() rs.status()
